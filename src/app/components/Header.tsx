...
export default function Header(): JSX.Element {
  ...
  // Determine menu background color based on dark mode
  const menuBgColor = isDarkMode ? 'bg-secondary' : 'bg-white';
    ...
    <>
      ...
      {isMenuOpen && (
        <motion.div 
          ...
          animate={{ opacity: 1 }}
          exit={{ opacity: 0 }}
          className="fixed inset-0 bg-black/60 dark:bg-black/70 z-[90] md:hidden mobile-menu-overlay"
      ...
      <motion.div
        ...
        className={`fixed top-[4rem] left-0 right-0 ${menuBgColor} dark:border-t dark:border-white/10 backdrop-blur-lg md:hidden overflow-hidden shadow-lg z-[95] mobile-scroll`}
      >
        <nav className="container mx-auto py-6 px-4 overflow-y-auto h-full">
          ...
          <motion.div
            variants={menuItemVariants}
            className={`mt-10 pt-6 border-t ${menuBorderColor}`}
          >
            <p className={`text-sm ${isDarkMode ? 'text-white/60' : 'text-foreground/60'} mb-4`}>Connect with us</p>
            <div className="flex space-x-4">
              <a href="#" className={`w-10 h-10 rounded-full ${isDarkMode ? 'bg-white/10' : 'bg-gray-100'} flex items-center justify-center ${menuTextColor}`}>
